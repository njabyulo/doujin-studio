...
export function applyEditorCommand(
  ...
  if (command.type === "setVolume") {
    const located = findClip(data, command.clipId);
    if (!located || located.clip.type === "subtitle") {
      return data;
    }
      ...
      tracks: data.tracks.map((track) => ({
        ...
        clips: track.clips.map((clip) =>
          clip.id === command.clipId
            ? { ...clip, volume: clamp(command.volume, 0, 2) }
            : clip,
        ),
      })),
    });
  ...
  if (command.type === "addSubtitle") {
    const subtitleTrack = data.tracks.find((track) => track.id === command.trackId);
    if (!subtitleTrack || subtitleTrack.kind !== "subtitle") {
      return data;
    }
